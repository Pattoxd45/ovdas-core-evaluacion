...
def preprocessing(signal_data: np.ndarray, spect_size: tuple[int, int] = (112, 112)):
    """
...
os.listdir()
data_dir = "DATOS/trazas_small/LP/"
traza_dict = {}
for idx, path in enumerate(os.listdir(data_dir)[:20]):
    traza_dict[idx] = np.load(data_dir + path)
ev_dict = {}
for idx in traza_dict.keys():
    ev_dict[idx] = len(traza_dict[idx])

# 1.- Definir directorio de pesos
weights_path = "PESOS/rep2_3classes_weights.pt"
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

model = load_model(weights_path, device)
OOD_path = "OOD_detector_test.pkl"
OOD_detector = load_OOD_detector(OOD_path)
# Arreglo 1D con información de la traza
input_signal = 0
input_signal = np.empty(0)
for idx in traza_dict.keys():
    input_signal = np.concatenate([input_signal, traza_dict[idx]])

# Lista de tuplas [[indice_inicio1, indice_fin1],[indice_inicio2, indice_fin2],...] para cada evento presente en la traza
ev = [[0, 4000]]
ev = []
ev1 = 0
for idx in ev_dict.keys():
    ev2 = ev_dict[idx] + ev1
    ev.append([ev1, ev2])
    ev1 = ev2
...
tpclass = predict(input_signal, ev, model, OOD_detector)
tpclass

# temp_signal = input_signal[0:ev1]
# temp_input = input_gen(temp_signal)
